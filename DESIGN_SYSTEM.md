# ldj portfolio · Design System v1.3
...
Place DESIGN_SYSTEM.md at your project root (e.g. ~/dev/ldj-portfolio/).
Place tokens.css in your styles folder and import it into globals.css.
When prompting Claude Code, add:
...
- Dark minimal base — #0A0A0A background, near-black but slightly warm. Never pure #000000.
- One accent, sparingly used — Signal orange #FA5112 on less than 10% of any view. Reserved for hover/active states, numbered badges, key highlights, and punctuation accents (the period after display titles).
- Extreme typography scale — Display (72-96px) + body (14-15px). No mid-sized headings. Hierarchy through scale contrast, not weight.
- Sentence-level restraint — English UPPERCASE for section labels only. Korean body stays regular case.
- Whitespace first — Sections separated by 120-200px vertical space. Never cramp.
...
A short vertical tick + small neon dot in the middle of a horizontal rule. Marks major transitions.
...
All case studies follow these five sections in order:
...
### Aspect ratio & dimensions by context

| 용도 | 비율 | 권장 크기 | 비고 |
|---|---|---|---|
| Cover Image (카드 썸네일) | 4:5 세로 | 1200×1500 이상 | 포트폴리오 인덱스에 노출 |
| Detail / Gallery (상세 페이지 본문) | 16:9 가로 | 최대 1920×1080 | 데스크탑 스크린샷·UI·프로젝트 화면 |
| Square (Instagram 포맷) | 1:1 | 1080×1080 | SUPPL 카루셀 등 소셜 호환 필요 시 |

**상세 페이지 이미지 제작 시**
- 기준 크기: 1920×1080px (16:9)
- 업로드 최대: 1920×1080 넘지 않기
- 최소 권장: 1440×810 이상이면 Retina에서도 선명
- 파일 형식: WebP 우선, JPG 허용, PNG는 UI 스크린샷에만
...
- Aspect ratio consistency: 한 페이지 내에서는 비율 통일
